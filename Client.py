...
class Client:
    ...
    def run(self):
        try:
            while True:
                ...
                elif choice == "6" and self.access_level == 1:  # and (0 <= int(self.access_level) <= 1):
    ...
    def register_user(self):
        ...
        while not self.validate_password(p):
            p = input("Enter your password: ")
        password = p
    ...
    def enter_group_chat(self):
        ...
        #     name_add = input("Enter a username: ")
        #     self.send_message(f"{name_add},{group_name}")
        #     print(f"Removed {name_add} from {group_name}")
...
#             print(f"Entered group {group_name}")
#             self.send_message(group_name)
#             # Checks the group port with server's public key
#             # Receive and set the peer's public key
#             server_public_key_pem = self.receive_message()
#             server_public_key = server_public_key_pem.encode()
#             self.send_message("KEY ACK")
#             # Verify the message
#             parts = self.receive_message()
#             nonce, aes_key, ciphertext, tag, hmac_tag, signed_message = parts.split(":")
#             nonce = base64.b64decode(nonce)
#             aes_key = base64.b64decode(aes_key)
#             ciphertext = base64.b64decode(ciphertext)
#             tag = base64.b64decode(tag)
...
#                 peer_rsa_key = RSA.import_key(server_public_key)
#                 data_to_verify = nonce + aes_key + ciphertext + hmac_tag
#                 h = SHA256.new(data_to_verify)
#                 try:
#                     pkcs1_15.new(peer_rsa_key).verify(h, signature)
...
#                     final_message = decrypted_message.decode('utf-8')
#                     # print(final_message)
#                     group_port = int(final_message)
#                     print("Received group port", group_port)
#                     address = 'localhost'
#                     self.send_message("ACK CORRECT")
#                 except (ValueError, TypeError) as e:
#                     print("Signature verification failed.", str(e))
#                     self.send_message("ACK FAILED")
#                     return
#             self.p2p_chat(address, group_port)
#         elif command == 2 and access_level == 1:
#             name_add = input("Enter a username: ")
