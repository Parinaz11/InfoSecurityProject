...
class Client:
    ...
    def run(self):
        try:
            while True:
                ...
                if choice == "1":
                    self.register_user_client()
                elif choice == "2":
                    self.login_user_client()
                elif choice == "3":
                    print("Exiting...")
                    break
                elif choice == "4":
    ...
    def register_user_client(self):

        global enteredThread  # Declare the global variable
        ...
        print("MAIN HEREEE")
        message = None
        while not enteredThread and message == None:
            print("Waiting")
            message = self.receive_message()
            print(message)
        enteredThread = False
        print("MAIN HEREEE 222222222222222")

    def login_user_client(self):
    ...
    def start_p2p_server(self, port):
    ...
    def handle_server_messages(self):
        ...
        while True:
            ...
            message = self.receive_message()
            print("GOTTT", message)
            if message.startswith("START_P2P_SERVER"):
                _, port = message.split(":")
                port = int(port)
                self.p2p_server_thread = threading.Thread(target=self.start_p2p_server, args=(port,))
...
def main():
    client = Client()
    threading.Thread(target=client.handle_server_messages, daemon=True).start()
    client.run()
